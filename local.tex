...
% Chinese support via CJKutf8 for pdflatex.
% CJKutf8 4.8.4 replaces the modern NFSS \selectfont implementation with a
% pre-2020 version that ignores delayed series changes, making \textbf and
% \bfseries ineffective. Preserve modern NFSS font selection and retain the
% CJK-specific post-selection step through the standard selectfont hook.
\makeatletter
\expandafter\let\expandafter\ModernNFSSselectfont\csname selectfont \endcsname
\makeatother
\usepackage{CJKutf8}
\makeatletter
\expandafter\let\csname selectfont \endcsname\ModernNFSSselectfont
\AddToHook{selectfont}[cjkutf8-compat]{%
...
% Scale the nearest RSFS design size instead of substituting a discrete size.
...
% Inconsolata exposes its italic face as scit; make \itshape use that face.
\makeatletter
\InputIfFileExists{t1zi4.fd}{}{}
\makeatother
\DeclareFontShape{T1}{zi4}{m}{it}{<-> ssub * zi4/m/scit}{}
...
% acmart loads xcolor; main.tex passes dvipsnames before \documentclass.
\usepackage{xcolor}
% \usepackage{pdfpages}
\colorlet{RED}{red}
% \usepackage{subfig}
...
\usepackage{pifont}
\usepackage{array}
% The table option is passed to xcolor before \documentclass in main.tex.
