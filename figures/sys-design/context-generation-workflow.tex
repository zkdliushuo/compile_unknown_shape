...
\begin{tikzpicture}[
    ...
    % 2. Process: Extract Parameter Types (box style)
    \node[box] (extract) at (3.2, 0) {{\small\ding{173}} Extract\\Param Types};

    % 3. Intermediate Data: Parameter Type Sequence Phi (doc style)
    ...
    % Edges
    \draw[->] (input) -- (extract);
    \draw[->] (extract) -- (param_types);
    \draw[->] (param_types) -- (update);
    \draw[->] (update) -- (output);
...
\Description{Flow diagram showing the context generation process from Graph IR Node through Parameter Type Extraction to Call Context.}
