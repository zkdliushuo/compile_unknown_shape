...
\begin{itemize}
    ...
    \item \textbf{Optimal:} An oracle approach denoting the performance upper bound. For each specific input shape, we first execute the operator to capture the actual runtime values of the schedule parameters passed to the kernel. We then perform constant substitution on the kernel's LLVM IR using these captured values and apply the kernel specialization module of \mysys.
\end{itemize}
...
Overall, \textbf{\mysys} achieves a geometric mean speedup of \textbf{1.06$\times$} across all operators, significantly outperforming the \textbf{LLVM-spec} baseline (1.01$\times$). Crucially, the performance of \textbf{\mysys} is nearly identical to the \textbf{Optimal} oracle (within 0.5\% difference on average), demonstrating that our framework effectively captures and leverages almost all available runtime constant information for specialization without the heavy overhead of full compilation for each possible shape.
...
The end-to-end model inference performance is depicted as \autoref{fig:eval:model-overall}, showing normalized speedup for \textbf{Llama2-7B} and \textbf{Qwen2-1.5B} across varying sequence lengths and batch sizes (BS=1, 2, 4) during both Prefill and Decode phases.

\textbf{\mysys} improves inference speed across all sequence lengths and batch sizes, with speedups ranging from \textbf{1.01$\times$ to 1.04$\times$}.
The end-to-end speedup is smaller compared to average kernel speedups because of non-computational overheads such as kernel launching, memory copying, and binary operators.
...
\begin{figure}
    ...
    \caption{Overall normalized performance of \mysys compared to PyTorch baseline based on CANN compiler across various dynamic shape operators. For the prefill phase, we measure the time to first token (TTFT); for the decode phase, we measure the average time per output token (TPOT).}
