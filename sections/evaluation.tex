...
For the GPU library debloating experiment, we use a platform equipped with an NVIDIA RTX 34090 GPU with 24GB memory and an Intel Xeon Gold 6348 CPU.
The version of CUDA is 12.2 and the version of PyTorch is 2.6.0.
The version of vLLM is 0.9.1 and the version of FlashInfer is v0.2.
...
While our primary evaluation focuses on Ascend NPUs, we also explore the applicability of \mysys to other accelerators, specifically GPUs.
Applying \mysys to optimize kernel execution latency on GPUs presents unique challenges compared to NPUs.
Modern GPUs are equipped with powerful scalar computation capabilities (e.g., via CUDA Cores) and abundant register resources.
More importantly, mainstream GPU operator libraries have evolved to be highly mature.
Critical parameters that impact control flow and pipeline scheduling are often manually specialized through C++ templates.
By enumerating all possible values of these template parameters, these libraries instantiate multiple kernel versions at compile time and have achieved full loop unrolling and optimal instruction scheduling, leaving limited room for the runtime scalar propagation optimization that \mysys excels at on NPUs.

However, this exhaustive specialization strategy comes at a cost of binary bloat as shown in \autoref{lst:compare_aot_jit_without_graph}.
To support a wide range of models and hardware configurations, a single kernel function often preserves many template parameters, leading to it being instantiated into hundreds versions.
But in a specific serving scenario, the model architecture and hardware are fixed and only a small subset of these instantiated kernels are actually invoked, leading to the vast majority of these kernels dead code.
This redundancy results in excessive disk usage and increased library loading time, especially increase storage and memory footprint on resource-constrained edge devices and serverless environments.

We adapted \mysys to address this issue by eliminating dead code through inter-procedural propagation on the GPU operators.
...
Instead of optimizing the kernel code itself, we utilized \mysys to construct the precise invocation context from the PyTorch frontend based on the target LLM configuration (e.g., Llama2-7B or Qwen2).
This context is propagated to the host-side C++ code of the operator same as in \autoref{sec:design:context_construction}.
By analyzing the host-side dispatch logic with the provided context, \mysys identifies the specific set of kernels that are actually reachable.
We then re-compile the library, stripping away the definitions and call sites of all unreachable kernel instantiations.
...
In summary, \mysys remains a powerful tool imporoves the performance of operator libraries on different DSA architectures, whether for latency reduction on NPUs or storage efficiency on GPUs.
