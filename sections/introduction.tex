...
status add(Tensor& A, Tensor& B, Tensor& C) {
  ...
  return kernel(A.ptr, B.ptr, C.ptr, s, t);
    ...
    \texttt{Tensor} is a data structure representing a multi-dimensional array adopted in deep learning frameworks, with a method \texttt{size} that retrieves the size of a specific dimension.\scnote{\kw{add} returns status, but \kw{kernel} returns void.}}
...
\autoref{lst:invariable_params_scalar_ratio} shows some representative operators whose 
