...
Combined with tile size configurations, this results in over 1000 binary instantiations, yet a specific model \DIFdel{like \textbf{Qwen2-1.5B} }utilizes only $\sim$10 kernels.
...
\DIFadd{Besides, the further sensitivity study in \S\ref{sec:eval:sensitivity} analyzes which schedule parameters tend to have the most significant impact on performance. It reveals that a small subset of parameters, which lie in the hot path of the kernel, are responsible for the majority of performance impact.}\par
