...
Two primary methods are currently employed for specializing kernel parameters in DSA operator libraries: Ahead-of-Time (AoT) compilation and Just-in-Time (JIT) compilation.
...
status aot_add(Tensor& A, Tensor& B, Tensor& C) {
  ...
  switch (t) {
    case 16: kernel<16>(..., A.szie[0]); break;
    case 32: kernel<32>(..., A.szie[0]); break;
}}
void jit_add(Tensor& A, Tensor& B, Tensor& C) {
  int t = 0; tiling(A.size[0], t);
  auto kernel_ = run_compile(kernel, t);
  kernel_(A.ptr, B.ptr, C.ptr, A.szie[0]);
}
\end{lstlisting}
\end{minipage}
\caption{Comparison of AoT compilation and JIT compilation for kernel specialization of add operator. For simplicity, we omit error handling and the definition of the \kw{tiling} function.}
