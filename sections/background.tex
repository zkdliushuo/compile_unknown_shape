...
Schedule-generic kernels support complex scheduling strategies by receiving numerous schedule parameters from the host at runtime.
...
Combined with tile-size configurations, this results in over 1,000 binary instantiations, yet a specific model uses only $\sim$10 kernels.
...
In addition, the sensitivity study in \S\ref{sec:eval:sensitivity} analyzes which schedule parameters have the greatest effect on performance.
It shows that a small subset of parameters on the kernel's hot path accounts for most of the performance gain.
...
This mapping is not a direct binding between the arguments of a Graph IR node and C++ parameters.
