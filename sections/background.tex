...
% \usepackage{xcolor} % Include xcolor for syntax highlighting in listings
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, caption={Offline compilation.}]
...
\end{lstlisting}
\label{lst:offline_compilation}
\end{minipage}
\hfill
\begin{minipage}{0.47\textwidth}
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, caption={Runtime JIT compilation.}]
void add_kernel(float* A, float* B, float* C, int N) {
...
The above challenges necessitate a specialized constant propagation framework tailored for C++-based operator libraries, which motivates us to develop \mysys.